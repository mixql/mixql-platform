...
Universal / mappings := {
  ...
  val platformOozieFiles = ((Universal / mappings).value :+
    file("README.md") -> "README.md" :+
    applicationConf -> "application.conf" :+
    log4jConf -> s"${log4jConf.getName}").map(
    t => t._1 -> t._2.replaceFirst("lib/", "")
  ).distinct.filter(
    t => if (t._2.startsWith("bin/"))
      false
    else
      true
  )
  ...
  ///Generate shell///
  {
    ...
    PlatformOozieShell.gen_shell(target, platformClassName,
      platformOozieFiles.map(
        t => t._2
      ).distinct.toList
    )
  ...
  ///Generate workflow///
  {
    ...
    val platformClassName = "org.mixql.platform.oozie.MixQlEnginePlatformOozie"
    PlatformOozieWorkflow.genWorkflow(target, name.value, version.value,
      allFiles.map(
        t => t._2
      ).distinct.toList
    )
