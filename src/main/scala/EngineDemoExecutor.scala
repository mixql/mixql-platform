...
object EngineDemoExecutor extends IModuleExecutor {
  val engineParams: mutable.Map[String, scalapb.GeneratedMessage] = mutable.Map()
  ...
  def reactOnMessage(msg: Array[Byte])
    ...
    ProtoBufConverter.unpackAnyMsg(msg) match {
      case clientMsgs.Execute(statement, _) =>
        println(s"Module $identity: Received Execute msg from server statement: ${statement}")
      ...
      case clientMsgs.SetParam(name, value, _) =>
        try {
          println(s"Module $identity :Received SetParam msg from server $clientAddressStr: " +
            s"must set parameter $name ")
          engineParams.put(name, ProtoBufConverter.unpackAnyMsg(value.get.toByteArray))
          println(s"Module $identity: Sending reply on SetParam  $name msg")
          sendMsgToServerBroker(clientAddress, clientMsgs.ParamWasSet())
        } catch {
          case e: Throwable => sendMsgToServerBroker(clientAddress,
            clientMsgs.Error(s"Module $identity to ${clientAddressStr}: error while executing Set Param command: " +
              e.getMessage
            ))
        }
      case clientMsgs.GetParam(name, _) =>
        ...
        try {
          ...
          case e: Throwable => sendMsgToServerBroker(clientAddress,
            clientMsgs.Error(s"Module $identity to ${clientAddressStr}: error while executing get Param command: " +
              e.getMessage
            )
          )
        }
