...
object StubSimpleProc {
  ...
  val simple_func_context_params = new((StubContext, String, Int) => String) {
    override def apply(ctx: StubContext, a: String, b: Int): String = {
      s"SUCCESS:${ctx.name}:$a:${b.toString}"
    }
  }

  val simple_func_return_arr = new((StubContext) => Array[Array[String]]) {
    override def apply(ctx: StubContext): Array[Array[String]] = {
      Array(
        ...
        Array("2", "Ana Trujillo Emparedados y helados", "Ana Trujillo", "Avda. de la Constitución 2222",
  ...
  val simple_func_return_map = new((StubContext) => Map[String, Map[String, String]]) {
